...
# ---------------------------------------------------------
st.sidebar.header("🔐 نظام الاشتراك والتفعيل")
license_input = st.sidebar.text_input("أدخل رمز التفعيل الخاص بك:", type="password", placeholder="مثال: FIELD-PRO-2026")

# قائمة الرموز المفعلة التجريبية (يمكن ربطها لاحقاً بقاعدة بيانات أو نظام دفع حقيقي)
valid_licenses = ["FIELD-PRO-2026", "VIP-TUNISIA-99", "DEMO-TEST-KEY"]

is_subscribed = license_input in valid_licenses

if is_subscribed:
    st.sidebar.success("✅ الاشتراك مفعل بنجاح (باقة المحترفين)")
else:
    st.sidebar.warning("⚠️ أنت تستخدم النسخة المجانية المحدودة")
    st.sidebar.info("💡 للحصول على رمز تفعيل كامل الميزات والتقارير الاستشارية، تواصل معنا للاشتراك بقيمة 50 دينار.")
...
# توليد النقاط الحرارية (تظهر للمشتركين أو كمعاينة أساسية)
...
with m_col3:
    st.metric(label="📏 العمق التقديري", value=f"{est_depth} م" if is_subscribed else "🔒 مخفي (مفعل للاشتراك)")
with m_col4:
    st.metric(label="🌡️ البصمة الحرارية", value=f"{est_temp} °C" if is_subscribed else "🔒 مخفي (مفعل للاشتراك)")

# عرض الخريطة بأبعاد عملاقة جداً (920 بكسل)
from streamlit_folium import st_folium
st_folium(m, width="100%", height=920, use_container_width=True)

# ---------------------------------------------------------
# التقرير النصي المعمق (مشروط بحالة الاشتراك)
# ---------------------------------------------------------
st.markdown("---")
...
if is_subscribed:
    ...
    * **معامل الثقة:** استناداً إلى دقة الإحداثيات ومعامل الحساسية المختار ({sensitivity}%), فإن نسبة ترجيح وجود شذوذ هيكلي حقيقي في هذا النطاق تُعتبر **إيجابية وذات أهمية استكشافية متقدمة**.
    ...
    # زر تصدير التقرير والبيانات (متاح للمشتركين فقط)
    st.markdown("---")
    ...
    st.markdown("""
        <div class="locked-box">
            <h3>🔒 التقرير الفني المعمق وتصدير البيانات مقفل</h3>
            <p>هذا المحتوى الاستشاري والتحليلي الكامل متاح فقط لحاملي باقة الاشتراكات المدفوعة.</p>
            <p>قم بإدخال رمز تفعيل صالح في الشريط الجانبي (أو تواصل معنا للحصول على اشتراكك بقيمة 50د) لفتح كافة ميزات المنصة والتقارير الميدانية.</p>
        </div>
    """, unsafe_allow_html=True)
